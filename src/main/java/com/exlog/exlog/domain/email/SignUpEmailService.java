...
public class SignUpEmailService {
    ...
    public void sendVerificationCode(String email) {
        ...
        message.setTo(email);
        message.setSubject("[Ex-Log] 회원가입 인증 번호입니다.");
        message.setText("인증 번호: " + code + "\n5분 이내에 입력해주세요.");

        javaMailSender.send(message);

        // Redis에 저장 (Key: EMAIL_AUTH:이메일, Value: 코드, TTL: 5분)
        stringRedisTemplate.opsForValue().set("EMAIL_AUTH:" + email, code, 5, TimeUnit.MINUTES);
    }
    ...
    public boolean verifyCode(String email, String inputCode) {
        String savedCode = stringRedisTemplate.opsForValue().get("EMAIL_AUTH:" + email);
